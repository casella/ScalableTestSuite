...
<p>The initial equations initialize the generator at steady-state, assuming that a load with the nominal power of the plant is consuming the entire production locally.</p>
    ...
    model PowerSystem
      ...
      SI.Power P_load[N](each nominal = 1e9) = ones(N)*P_nom
        "Active power consumed by loads - replace the default binding";
      SI.Power P_ex[N,N](each nominal = 1e9) "Power going from generator i to generator j";
      SI.Power P_diss[N,N](each nominal = 1e9) "Power dissipated by the generators i and j";
      SI.Power P_a[N](each nominal = 1e9) "Net active power out of generator i";
      SI.Power P_f(nominal = 1e9) = P_nom
        "Power factor of a single trunk of transmission line";
      SI.Power P_sfc(nominal = 1e9) "Additional power request for secondary frequency control";
